...
Mantiene todas las rutas del benchmark anterior, pero ahora montadas
bajo el prefijo /bench desde el app.py principal.

Todas las rutas que antes eran "/algo" ahora son "/bench/algo".
"""
import os
import json
import shutil
...
from vision_core.modelos.pose_svm import PoseSVMDetector
from vision_core.modelos.pose_xgb_norm import PoseXGBNormDetector
from vision_core.paths import YOLO_EXP5, YOLO_EXP8


# ============================================================
# CONFIGURACIÓN DE CARPETAS (relativas a este archivo)
# ============================================================
BENCH_DIR = Path(__file__).parent.resolve()
CARPETA_DEMOS = BENCH_DIR / "videos_demo"
META_DEMOS = CARPETA_DEMOS / "meta.json"

# Los videos subidos por el usuario van a una carpeta temporal en la raíz del proyecto
CARPETA_VIDEOS = BENCH_DIR.parent / "videos_benchmark"
...
MODELOS_DISPONIBLES = {
    ...
    "exp5": YOLOv8BinarioDetector(nombre="YOLOv8 Exp5", ruta_pesos=YOLO_EXP5),
    "exp8": YOLOv8BinarioDetector(nombre="YOLOv8 Exp8 (Data Aug)", ruta_pesos=YOLO_EXP8),
...
def listar_videos_demo():
    ...
    for demo in demos:
        ruta = CARPETA_DEMOS / demo["archivo"]
        if not ruta.exists():
            continue
        disponibles.append({
            "id": demo["id"],
...
def iniciar_sesion_demo(request: IniciarDemoRequest):
    ...
    ruta_video = CARPETA_DEMOS / demo["archivo"]
    if not ruta_video.exists():
        raise HTTPException(404, "El archivo del video demo no existe en disco")
...
def limpiar_videos_huerfanos():
    """Borra videos subidos viejos al arrancar. Lo llama app.py en startup."""
    if not CARPETA_VIDEOS.exists():
        return
    for archivo in CARPETA_VIDEOS.iterdir():
        try:
            archivo.unlink()
        except Exception:
            pass
